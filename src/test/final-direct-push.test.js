...
describe('final-direct-push', () => {
  it('burnout — covers else-if drop > 10 path (drop=11)', () => {
    // recent[0]=120, recent[2]=109, drop=11
    const exams = [
  ...
  it('taskEngine — covers unitErr line 59 (skip when no unit)', () => {
    // Mistakes with m.unit=undefined
    const exams = [
      { date: '2025-01', japanese: { reading: 100, listening: 100, wrongQuestions: { reading: [5, 6], listening: [3, 4] } }, comprehensive: { score: 70, mistakes: [{ noUnit: true }] } },
  ...
  it('subjectClassifier — covers fuzzyIncludes false for null keyword', () => {
  ...
  it('storage — setStorageProvider round-trip', () => {
    const mockProvider = {
      getItem: async () => null,
      setItem: async () => {},
      removeItem: async () => {},
